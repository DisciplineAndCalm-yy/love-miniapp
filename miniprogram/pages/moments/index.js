...
Page({
  ...
  async reload() {
    ...
    try {
      ...
      this.setData({ list: (res.list || []).map((x) => ({ ...x, time: fmt(x.createdAt), icon: doingIcon(x.doing) })), hasMore: res.hasMore })
  ...
  async more() {
    ...
    try {
      ...
      this.setData({ list: this.data.list.concat((res.list || []).map((x) => ({ ...x, time: fmt(x.createdAt), icon: doingIcon(x.doing) }))), hasMore: res.hasMore })
